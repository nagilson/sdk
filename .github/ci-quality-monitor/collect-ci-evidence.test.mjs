...
test("PR check-suite builds analyze once without bootstrap suppression", async () => {
  const build = {
    id: 42, result: "failed", reason: "pullRequest", sourceBranch: "refs/pull/123/merge",
    sourceVersion: "abc", finishTime: "2026-07-24T14:00:00Z",
    definition: { id: 101, name: "dotnet-sdk-public-ci" }, repository: { id: "dotnet/sdk" }, validationResults: []
  };
  const responses = new Map([
    ["/build/builds/42?", build], ["/build/builds?", { value: [build] }],
    ["/build/builds/42/timeline?", { records: [] }], ["/runs?", { value: [] }]
  ]);
...
test("check-suite events silently ignore non-PR Azure builds", async () => {
  const build = {
    id: 43, result: "failed", reason: "batchedCI", sourceBranch: "refs/heads/main",
...
test("neutral PR suites resolve YAML failures by source head SHA", async () => {
  const build = {
    ...
    sourceVersion: "merge-sha", finishTime: "2026-07-24T15:00:00Z",
