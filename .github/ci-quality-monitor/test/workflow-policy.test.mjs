...
test("named test assertions require the validated recurring KBE gate", async () => {
  const workflow = await readFile(workflowUrl, "utf8");

  assert.match(workflow, /A named test assertion \(`kind: test`, `failureType: test-assertion`\) may create an issue only through the validated recurring Known Build Error gate below\./);
